...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="flex h-screen w-full bg-[#F8FAFC] dark:bg-slate-950 overflow-hidden">
      ...
      {/* Main Content Area */}
      <div className={cn(
        "flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-900 shadow-glass overflow-hidden border-slate-100 dark:border-slate-800 transition-all duration-300",
        !isMobile ? "rounded-tl-[1.5rem] mt-2 border-l border-t" : "mt-0 border-none"
      )}>
        {/* Top Header */}
        <header className="h-16 md:h-20 px-4 md:px-8 flex items-center justify-between bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm border-b border-slate-50 dark:border-slate-800">
          <div className="flex items-center gap-4 flex-1">
            ...
            {isMobile && (
              <div className="font-bold text-primary text-lg">GestãoPro</div>
            )}
          </div>
